...
function doChopWood() {
	...
	if (actor.moves <= 0) {
		writeHistory_(ss, actor.name, "👣0", "Нет ходов", "", "");
		return;
	}
	...
	writeHistory_(ss, actor.name, `🪵+${gained}${bonus ? " (🪓)" : ""}`, `${tile}→${nextTile}`, "", timerInfo);

	moveAnimals_(
		ss,
		gridRange_(getSheet_(ss, CFG.SHEETS.base)).getNumColumns(),
		gridRange_(getSheet_(ss, CFG.SHEETS.base)).getNumRows(),
	);
	updateFog();
}
...
function doQuarry() {
	...
	writeHistory_(ss, actor.name, `🪨+${gained}${bonus ? " (⛏️)" : ""}`, `${tile}→${nextTile}`, "", timerInfo);
...
function doHunt() {
	...
	writeHistory_(ss, actor.name, `🍖+${gained}`, `${huntableTile}→⬜️`, "", animal ? "" : `⏱️${CFG.REGEN_DAYS.hunt}`);
