...
function moveAnimals_(ss, w, h) {
	...
	const zombieTile = CFG.ZOMBIE?.aliveTile;
	...
	animals = animals.map((a) => {
		...
		for (const d of shuffled) {
			...
			if (CFG.BLOCKED?.has(tile)) continue;
...
function resolveNpcFight_(ss, actor, npc) {
	...
	writeHistory_(
		ss,
		actor.name,
		`❤️-${dmg} (HP:${newHp}/${actor.maxhp || 10})`,
		`Получил удар от ${npc.emoji}`,
		diceInfo,
		"",
	);
	if (newHp <= 0) handleDeath_(ss, actor.name, npc.emoji);
...
function killNpc_(ss, actor, npc, diceInfo) {
	const npcs = getNpcs_().filter((n) => !(n.x === npc.x && n.y === npc.y));
	setNpcs_(npcs);
	...
	writeHistory_(
		...
		actor.name,
		got,
		`⚔️ Убил ${npc.emoji} (HP:${spec ? spec.hp : npc.hp})`,
		`${diceInfo} | ❤️ ${actor.hp}/${actor.maxhp || 10}`,
		"",
	);
	syncToolFlags_(ss);
}
