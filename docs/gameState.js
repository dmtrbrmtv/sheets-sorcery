...
export function createInitialState() {
  ...
  hunters.forEach(h => logHistory(state, "🌍", `${h.emoji} ${h.name} вышел на охоту (${h.x},${h.y})`, { moveInDay: 0 }));
...
function spawnAnimals(world, occupied = new Set()) {
  ...
  const maxCount = (CFG.ANIMALS && CFG.ANIMALS.maxCount) || 8;
...
function spawnVillagers(world, occupied = new Set()) {
  ...
  for (const [emoji, spec] of entries) {
    for (let tries = 0; tries < 20; tries++) {
      ...
      villagers.push({ x, y, emoji, name: spec.name || emoji, role: spec.role || "resident", dialog: spec.dialog || "..." });
      occupied.add(key);
      break;
    }
  }
    ...
    for (const d of shuffled) {
      ...
      if (animal) {
        ...
        if (!onAction || visible?.has(`${hunter.x},${hunter.y}`) || visible?.has(key)) {
          logHistory(state, "🌍", `${hunter.emoji} ${hunter.name} подстрелил ${animal.emoji} на (${nx},${ny})`, { moveInDay: 0 });
        }
      ...
      occupied.add(key);
      if (!onAction || visible?.has(`${hunter.x},${hunter.y}`) || visible?.has(key)) {
        logHistory(state, "🌍", `${hunter.emoji} ${hunter.name} перешёл (${hunter.x},${hunter.y}) → (${nx},${ny})`, { moveInDay: 0 });
      }
      moved = { ...hunter, x: nx, y: ny };
      break;
    }
