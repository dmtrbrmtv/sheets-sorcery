...
import { getTileAt, setTileAt, buildVisibleSet, moveAnimals, moveNpcs, moveHunters, spawnNewEnemies, isInVillage } from "./gameState.js";
...
export function getEffectiveAtk(state) {
  ...
  const weaponBonus = Math.max(0, ...items.map(i => {
...
function startCombat(state) {
  ...
  state.combat = c;
  state.combatLog = [];
  const enemyName = c.type === "zombie" ? "Зомби" : (c.type === "npc" ? (CFG.NPCS?.[c.target.emoji]?.name || c.target.emoji) : (CFG.ANIMALS?.[c.target.type]?.name || c.target.emoji));
  addCombatLog(state, `⚔️ Бой с ${enemyName}!`);

  const spec = enemy.type === "npc" ? CFG.NPCS?.[enemy.target.emoji] : enemy.type === "animal" ? CFG.ANIMALS?.[enemy.target.type] : null;
...
function killZombie(state) {
  ...
  addHistory(state, p.name, got, `⚔️ Убил 🧟 → ${CFG.ZOMBIE.graveTile}`, `❤️ ${p.hp}/${p.maxhp}`, `⏱️${CFG.ZOMBIE.respawnDays}`);
...
function killNpc(state) {
  ...
  loot.forEach(l => {
    if (l.type === "gold") { addResource(state, "gold", l.val); }
    if (l.type === "food") { addResource(state, "food", l.val); }
    if (l.type === "item") { (p.items = p.items || []).push(l.val); }
  });
...
export function doChopWood(state) {
  const enemy = getEnemyOnTile(state);
  if (enemy) { startCombat(state); return false; }
  const p = state.player;
  const tile = getTileAt(state.world, p.x, p.y, state);
  ...
  addHistory(state, p.name, `🪵+${gained}${toolIcon ? ` (${toolIcon})` : ""}`, `${tile}→${nextTile}`, "", `⏱️${CFG.REGEN_DAYS.wood}`);
  return true;
}
...
export function doQuarry(state) {
  ...
  let nextTile = tile === "🗻" ? "🪨" : tile === "🪨" ? "🧱" : tile === "🧱" ? "🕳️" : "🕳️";
  setTileAt(state.world, p.x, p.y, nextTile, state);
  if (!hasTimerAt(state, p.x, p.y)) addTimer(state, p.x, p.y, CFG.RESOURCES.STONE_REGEN_TO, CFG.REGEN_DAYS.stone, "stone");
  const toolIcon = bonus ? "⛏️" : "";
  addHistory(state, p.name, `🪨+${gained}${toolIcon ? ` (${toolIcon})` : ""}`, `${tile}→${nextTile}`, "", `⏱️${CFG.REGEN_DAYS.stone}`);
  return true;
}
...
export function doHunt(state) {
  ...
  else { setTileAt(state.world, p.x, p.y, "⬜️", state); addTimer(state, p.x, p.y, huntable, CFG.REGEN_DAYS.hunt, "hunt"); }
...
export function buildHouse(state) {
  ...
  const spent = {};
  for (const k in cost) { addResource(state, k, -cost[k]); spent[k] = -(cost[k]); }
  ...
  for (const ev of events) {
    ...
    if (r <= 0) {
      ...
      if (ev.quest) {
        ...
        if (def && !active && !completed) state.activeQuests = (state.activeQuests || []).concat([{ id: ev.quest, progress: {} }]);
      }
      return;
    }
  }
...
const CRAFT_SPECS = [
  { item: "🪓", name: "Топор", category: "item", cost: { wood: 2, stone: 1 }, woodBonus: 1, effect: "🪵 +1 дерево", tag: "common" },
  { item: "⛏️", name: "Кирка", category: "item", cost: { wood: 1, stone: 2 }, stoneBonus: 1, effect: "🪨 +1 камень", tag: "common" },
  { item: "🗡️", name: "Меч", category: "weapon", cost: { wood: 2, stone: 2, gold: 1 }, stat: { atk: 2 }, effect: "⚔️ +2 атака", tag: "common" },
  { item: "⚔️", name: "Двойные клинки", category: "weapon", cost: { wood: 4, stone: 3, gold: 2 }, stat: { atk: 4 }, effect: "⚔️ +4 атака · без щита", isNoShield: true, tag: "rare" },
  { item: "⚒️", name: "Молот и булава", category: "weapon", cost: { wood: 3, stone: 3, gold: 2 }, stat: { atk: 3 }, effect: "⚔️ +3 атака · оглушение · без щита", isNoShield: true, tag: "rare" },
  { item: "🔱", name: "Трезубец", category: "weapon", cost: { wood: 3, stone: 2, gold: 3 }, stat: { atk: 4 }, effect: "⚔️ +4 атака · дальний · без щита", isNoShield: true, tag: "rare" },
  { item: "🏹", name: "Лук", category: "weapon", cost: { wood: 2, stone: 1, gold: 1 }, costItems: { "🪢": 1 }, stat: { atk: 2 }, effect: "⚔️ +2 атака · удар на 1 клетку", tag: "common" },
  { item: "🛡️", name: "Щит", category: "item", cost: { wood: 2, stone: 2 }, stat: { armor: 2 }, isShield: true, effect: "🛡️ +2 защита", tag: "common" },
  { item: "🔰", name: "Крепкий щит", category: "item", cost: { wood: 4, stone: 4, gold: 2 }, stat: { armor: 4 }, isShield: true, effect: "🛡️ +4 защита", tag: "rare" },
  { item: "🧥", name: "Броня", category: "item", cost: { wood: 4, stone: 3, gold: 3 }, stat: { armor: 5 }, effect: "🛡️ +5 защита", tag: "rare" },
  { item: "🪖", name: "Шлем", category: "item", cost: { wood: 2, stone: 2, gold: 1 }, stat: { armor: 3 }, effect: "🛡️ +3 защита", tag: "common" },
  { item: "⛵", name: "Лодка", category: "item", cost: CFG.BOAT?.cost || { wood: 4, stone: 2 }, effect: "🌊 плавание по воде", tag: "common" },
  { item: "🧪", name: "Зелье", category: "item", cost: { herb: 2, fish: 1, gold: 1 }, consumable: true, heal: 2, effect: "❤️ +2 здоровье", tag: "common" },
  { item: "🍱", name: "Рационы", category: "item", cost: { food: 2, wood: 1 }, consumable: true, heal: 2, effect: "❤️ +2 здоровье", tag: "common" },
  { item: "🪢", name: "Верёвка", category: "item", cost: { wood: 2 }, effect: "⛰️ скалы · для лука", tag: "common" },
  { item: "💍", name: "Кольцо", category: "item", cost: { stone: 1, gold: 2 }, stat: { luck: 2 }, effect: "🍀 +2 удача · уворот", tag: "rare" },
];
