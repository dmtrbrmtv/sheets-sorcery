...
export function getCombatEnemyInfo(state) {
	...
	if (!c) return null;
	if (c.type === "zombie")
		return { emoji: "🧟", name: "Зомби", hp: c.hp ?? CFG.ZOMBIE.hp, maxHp: CFG.ZOMBIE.hp, armor: 0 };
	if (c.type === "npc") {
		const spec = CFG.NPCS?.[c.target.emoji];
		return {
			emoji: c.target.emoji,
			name: spec?.name || c.target.emoji,
			hp: c.target.hp,
			maxHp: spec?.hp ?? 4,
			armor: 0,
		};
...
export function getCombatDisplayData(state) {
	...
	const playerEffects = (p.statusEffects || [])
		.map((e) => {
...
export function getContextHints(state) {
	...
	if (enemy) {
		...
		return {
			...
			tileName:
				enemy.type === "zombie"
					? "Зомби"
					: CFG.NPCS?.[enemy.target?.emoji]?.name || CFG.ANIMALS?.[enemy.target?.type]?.name || "?",
			shortActions,
		};
	}
	...
	if (villager) {
		hints.push(`💬 Поговорить с ${villager.name}`);
		shortActions.push("Talk");
	}

	if (CFG.RESOURCES.WOOD_TILES.has(tile)) {
		hints.push("🪓 Рубка (+1 с топором)");
		shortActions.push(ACTION_SHORT.chop);
	}
	if (CFG.RESOURCES.STONE_TILES.has(tile)) {
		hints.push("⛏️ Каменоломня (+1 с киркой)");
		shortActions.push(ACTION_SHORT.quarry);
	}
	const huntable = animal ? animal.emoji || "🐇" : tile;
	if (CFG.RESOURCES.HUNT_TILES.has(huntable)) {
		hints.push("🏹 Охота");
		shortActions.push(ACTION_SHORT.hunt);
	}
	if (near.some((t) => t === "🌊")) {
		hints.push("🎣 Рыбалка");
		shortActions.push(ACTION_SHORT.fish);
	}
	if (tile === "🪧") {
	...
	if (tile === "🌊" && hasBoat) {
		hints.push("⛵ Плавать");
		shortActions.push(ACTION_SHORT.sail);
	}
	...
	let tileName = animal
		...
		: villager
			? villager.name
			: npc
				? CFG.NPCS?.[npc.emoji]?.name || npc.emoji
				: getTerrainName(tile);
