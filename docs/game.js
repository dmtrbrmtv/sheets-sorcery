...
import { GRID_W, GRID_H, CELL_SIZE_PX, CFG, DEBUG_SHOW_BIOMES } from "./config.js";
import { WORLD } from "./world_base.js";
import { createInitialState, getTileAt, buildVisibleSet, setTileAt, spawnHunters } from "./gameState.js";
import {
  move,
  doChopWood,
  doQuarry,
...
import { getTerrainBg, getTerrainName } from "./terrain.js";
import { getBiomeFromTile, getBiomeLabel } from "./worldGenerator.js";
import { createHomeState, getHomeLayout, placePlot, plantVeg, plantFruit, addPig, collect, getHomeTileAt, getCropAt, getPigAt, moveInHome } from "./home.js";
...
function renderPlayerPanel() {
  ...
  const phaseIndicator = (s.allPhases || ["day", "dusk", "night", "dawn"]).map(p => {
...
function renderActionsOrCombat() {
  ...
  if (inCombat && combatPanelEl) {
    ...
    let actionButtons = "";
    if (potionCount > 0) actionButtons += `<button data-action="use-consumable" data-item="🧪">🧪 Использовать зелье (${potionCount})</button>`;
    if (rationsCount > 0) actionButtons += `<button data-action="use-consumable" data-item="🍱">🍱 Использовать рационы (${rationsCount})</button>`;
    ...
    if (isOnDreamHouseSpot()) {
      ...
      actionButtons = `<button data-action="buy-dream-house" ${!canBuy ? "disabled" : ""}>🏡 Купить дом мечты (${costStr})</button>` + actionButtons;
    }
    if (isOnHouseTile()) {
    ...
    actionsEl.querySelectorAll("[data-action]").forEach(btn => {
      ...
      btn.addEventListener("click", () => {
        ...
        if (action === "use-consumable") {
          ...
          if (item) performTurn_(() => { if (useConsumable(state, item)) moveEntitiesOnAction(state); });
          return;
        }
        if (action === "portal-home") {
          performTurn_(() => { if (portalHome(state)) moveEntitiesOnAction(state); });
          return;
        }
        performTurn_(() => {
          let ok = false;
  ...
  if (contextHintsEl && !inCombat) {
    ...
    let html = "";
    if (hints.length) {
      const tileLine = tileEmoji && tileName ? `<div class="hint-tile">${tileEmoji} ${tileName} → ${shortActions.join(", ")}</div>` : "";
...
function renderHistory() {
  ...
  historyEl.innerHTML = grouped.map(g => {
    if (g.type === "group") {
...
function renderCraft() {
  ...
  const specs = selectedCraftCategory
    ? allSpecs.filter(s => (s.category || "item") === selectedCraftCategory)
    : [];
  ...
  if (spec) {
    const costSpans = spec.costParts.map(p =>
      `<span class="craft-cost-part${p.missing ? " craft-cost-missing" : ""}">${p.text}</span>`
    ).join("");
...
function renderQuests() {
  ...
  questsEl.innerHTML = quests.map(q => `<div class="quest-item">📜 ${q.name}<br><small>${q.progressText}</small></div>`).join("");
}
