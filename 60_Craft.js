...
function craftAxe() {
	craft_({ item: "🪓", name: "Топор", cost: { wood: 2, stone: 1 } });
}
function craftPick() {
	craft_({ item: "⛏️", name: "Кирка", cost: { wood: 1, stone: 2 } });
}
function craftSword() {
	craft_({ item: "🗡️", name: "Меч", cost: { wood: 1, stone: 1, gold: 1 }, stat: { atk: +1 } });
}
function craftShield() {
	craft_({ item: "🛡️", name: "Щит", cost: { wood: 1, stone: 1 }, stat: { armor: +1 } });
}
