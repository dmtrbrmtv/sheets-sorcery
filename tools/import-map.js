#!/usr/bin/env node
/**
 * Import map from Excel "🗺 База" sheet into web/world_base.js
...
if (!sh) {
  ...
  sh = wb.Sheets[names.find(n => n.includes("База")) || names[0]];
}
...
for (let R = START_ROW; R < START_ROW + GRID_H; R++) {
  ...
  while (row.length < GRID_W) row.push("⬜️");
  data.push(row);
}

while (data.length < GRID_H) {
  data.push(Array(GRID_W).fill("⬜️"));
}
...
// 26×32 grid — generated by tools/import-map.js from Excel "🗺 База"
...
console.log(`Wrote ${OUTPUT} (${GRID_W}×${GRID_H})`);
process.exit(0);
